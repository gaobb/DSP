function imdb = setupVoc12(datasetDir, varargin)
...
imdb.imageDir = fullfile(datasetDir, ['VOC', opts.edition]);  

% Get the list of  train and val images
map = containers.Map() ;
j = 0 ;
for si = 1:numel(imdb.meta.sets)-1
  ...
  for i=1:length(names)
    ...
    imdb.images.name{j} = ['TrainJPEGImages/',names{i} '.jpg'] ;
...
for si = 3
  ...
  for i=1:length(names)
    ...
    imdb.images.id(j) = j ;
    imdb.images.set(j) = si ;
    imdb.images.name{j} = ['TestJPEGImages/',names{i} '.jpg'] ;
    %info = imfinfo(fullfile(imdb.imageDir, imdb.images.name{j})) ;
    %imdb.images.size(:,j) = [info.Width ; info.Height] ;
    %fprintf('\radded %s', imdb.images.name{j}) ;
  end
